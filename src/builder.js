...
const Builder = {
    ...
    bundlePackage(packageName, installPath) {
        let entryPoint = Builder.prepareRequire(packageName, installPath).split('/').slice(2).join('/');
        const bundleCommand = `npx webpack index=${installPath} --config webpack.config.js`;
